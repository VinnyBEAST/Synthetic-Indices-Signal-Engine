...
const SYMBOLS = [
  ...
  "VOL100S","VOL75S","VOL50S",
  "RDBULL","RDBEAR"
];
const TREND_WINDOW = 30;
const STATUS_INTERVAL = 5000; // refresh table every 5s
const TRADE_CHECK_INTERVAL = 1000; // update Time Elapsed every 1s
const TRADE_CLEAR_MS = 5 * 60 * 1000; // clear entries after 5 minutes
...
setInterval(()=>{
    console.clear();
    console.log(chalk.blue("📊 CapitalSignals (D) — Synthetics Overview\n"));
    console.log(`SYMBOL      PRICE      1H     30M     15M`);
    console.log('-----------------------------------------');
    SYMBOLS.forEach(symbol=>{
        const mgr = managers[symbol];
        const price = mgr.latestPrice.toFixed(2) || "N/A";
        // placeholder trend symbols (↑ green, ↓ red) for past 1h,30m,15m
        console.log(`${symbol.padEnd(10)} ${price.padEnd(10)} ${chalk.green('↑')}      ${chalk.green('↑')}      ${chalk.red('↓')}`);
    });

    console.log('\n📈 Active Entries (D) — live trades (clears after 5 min)\n');
